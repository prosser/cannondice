...
export class DiceBox {
  ...
  constructor(
    ...
    this.renderer = gl
      ...
      : new CanvasRenderer /*{ antialias: true }*/();
  ...
  bindMouse(
    ...
    bind(container, ['mouseup', 'touchend'], (ev: TouchEvent | MouseEvent) => {
      ...
      const vector = new Vector2(
        m.x - box.mouseStart.x,
        -(m.y - box.mouseStart.y)
      );
  ...
  drawSelector() {
    ...
    this.pane = new Mesh(
      new PlaneGeometry(this.size.x * 6, this.size.y * 6, 1, 1),
      new MeshPhongMaterial(DiceConsts.selectorBackColors)
    );
  ...
  findDieAtMousePosition(ev: MouseEvent | TouchEvent) {
    ...
    let intersects = new Raycaster(
      ...
      new Vector3(
        (m.x - this.clientSize.x) / this.aspect,
        (m.y - this.clientSize.y) / this.aspect,
        this.size.x / 9
      )
  ...
  reinit(container: HTMLElement, dimensions: ClientRect) {
    ...
    this.aspect = Math.min(
      this.clientSize.x / this.size.x,
      this.clientSize.y / this.size.y
    );
    this.scale =
    ...
    this.desk = new Mesh(
      new PlaneGeometry(this.size.x * 2, this.size.y * 2, 1, 1),
      new MeshPhongMaterial({ color: DiceConsts.deskColor })
    );
  ...
  private generateVectors(
    ...
    for (let sides of notation.set) {
      ...
      const pos = new Vector3(
        this.size.x * (vec.x > 0 ? -1 : 1) * 0.9,
        this.size.y * (vec.y > 0 ? -1 : 1) * 0.9,
        rng() * 200 + 200
      );
      ...
      const angle = new Vector3(
        -(rng() * vec.y * 5 + inertia * vec.y),
        rng() * vec.x * 5 + inertia * vec.x,
        0
      );
  ...
  private __selectorAnimate(threadid) {
    ...
    let dAngle =
      0.3 * timeDiff * Math.PI * Math.min(24000 + threadid - time, 6000) / 6000;
